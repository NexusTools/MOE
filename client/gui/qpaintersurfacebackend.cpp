...
bool next(QVectorIterator<RenderInstruction> i) {
    if(i.hasNext()) {
        i.next();
        return false;
    }
...
void QPainterSurfaceBackend::paint(QPainter& p) {
    ...
    RenderInstructions::Iterator i = pendingInstructions.begin();
    if(i != pendingInstructions.end())
        do {
#define inst (*i)
            switch(inst.type){
                ...
                case RenderInstruction::BeginRenderGLScene:
                {
                    ...
                    if(glBuffer->fbo->bind()) {
                        ...
                        if(glBuffer->shadow && !inShadowPass) {
                ...
                case RenderInstruction::AllocateGLModel:
                {
                    ...
                    qDebug() << "Allocating GLModel" << ptr << vectors.size();
    
                    GLModel* model = glModels.value(ptr);
                    if(!model) {
                        model = new GLModel;
                        model->vectors.setUsagePattern(QGLBuffer::StaticDraw);
                        model->colours.setUsagePattern(QGLBuffer::StaticDraw);
                    ...
                    if(model->colours.bind()) {
                        int size = colours.size()*sizeof(vec3);
                        if(model->colours.size() == size)
                            model->colours.write(0, colours.data(), size);
                        else
                            model->colours.allocate(colours.data(), size);
                    ...
                    if(!textCoords.isEmpty()) {
                        if(!model->textCoords.isCreated()) {
                            model->textCoords.setUsagePattern(QGLBuffer::StaticDraw);
                ...
                case RenderInstruction::RenderGLModel:
                {
                    ...
                    if(!inShadowPass) {
                        ...
                        // Use shadow matrix
                        //activeGLBuffer->shadowProgram.setUniformValue(model->shader->attrib.camMatrix, activeGLBuffer->camMatrix);
        ...
        } while(++i != pendingInstructions.end());
