...
void qcolorFromScriptValue(const QScriptValue &object, QColor &out)
{
    if(object.isString()) {
        ...
        if(namedColors.isEmpty()) {
            QFile reader(":/resources/rgb.txt");
...
QScriptValue qurlToScriptValue(QScriptEngine *engine, QUrl const &in)
{
    return engine->newQObject(new MoeUrl(in.isLocalFile() ? in.toLocalFile() : in.toString()), QScriptEngine::ScriptOwnership);
}

void qurlFromScriptValue(const QScriptValue &object, QUrl &out)
{
    if(object.isQObject()) {
        MoeUrl* mUrl;
        if((mUrl = qobject_cast<MoeUrl*>(object.toQObject()))) {
            out = mUrl->url();
            return;
        }
    }
    out = MoeUrl::locate(object.toString());
}
