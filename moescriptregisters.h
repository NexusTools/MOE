#ifndef MOESCRIPTREGISTERS_H
#define MOESCRIPTREGISTERS_H
...
QScriptValue renderRecorderToScriptValue(QScriptEngine *engine, RenderRecorder* const &in)
{
    return engine->newQObject(in);
}

void renderRecorderFromScriptValue(const QScriptValue &object, RenderRecorder* &out)
{
...
QScriptValue qrectToScriptValue(QScriptEngine *engine, QRect const &in)
{
    ...
    rect.setProperty("x", in.top());
    rect.setProperty("y", in.left());
...
{
    out.setTop(object.property("x").toInteger());
    out.setLeft(object.property("y").toInteger());
...
{
    qDebug() << "QRGB from" << object.toVariant();
